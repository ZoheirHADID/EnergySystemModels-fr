...
- ``w`` : Humidité absolue [g/kg_air_sec]
- ``F`` : Débit massique d'air humide [kg/s]
- ``F_dry`` : Débit massique d'air sec [kg/s]
- ``F_water`` : Débit d'eau d'humidification [kg/s]
- ``Q_th`` : Puissance thermique [kW]
- ``To_target`` : Température de sortie cible [°C]
- ``wo_target`` : Humidité absolue de sortie cible [g/kg_air_sec]
- ``HumidType`` : Type d'humidification ("adiabatique" ou "vapeur")
- ``Pv_sat`` : Pression de vapeur saturante [Pa]
