"""Generate fallback SVG previews only when real model plots are unavailable.

Important: when a library method already defines a plot (for example
``ch.plot()``), the documentation must use the real figure generated by that
method, not a hand-drawn preview. This helper is only for examples that cannot
be executed deterministically in the docs environment yet, such as PV examples
that depend on remote PVGIS weather retrieval.
"""
